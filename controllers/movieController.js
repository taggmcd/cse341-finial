...
const store = async (req, res) => {
  ...
  try {
    const { title, releaseYear, genres, runtime, rated, plot, posterUrl } =
      req.body;
...
const update = async (req, res) => {
  ...
  try {
    const movie = await Movies.findByIdAndUpdate(
      movieId,
      { title, releaseYear, genres, runtime, rated, plot, posterUrl },
      { new: true }
    );
...
const destroy = async (req, res) => {
  ...
  deleteItemAndSendMessage(req, res, Movies);
};
